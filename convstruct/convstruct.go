...
type tagParser func(s string) string
type tagGenerator func(name, typ, tag string) string

var (
	genTag                    tagGenerator = defaultGenTagFunc
	parseTag                  tagParser    = defaultParseTagFunc
...
// UsageCfg ... config convstruct some feature
...
func SetCustomParseTagFunc(f tagParser) {
	parseTag = f
}

// SetCustomGenTagFunc use user's custom tagGenerator
func SetCustomGenTagFunc(f tagGenerator) {
	genTag = f
}
